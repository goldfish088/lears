...
impl Png {
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<Chunk> {
        match ChunkType::from_str(chunk_type) {
            Err(_) => None,
            Ok(chunk_type) => {
                let mut ret_chunk: Option<Chunk> = None;
                for chunk in &self.chunks {
                    let other_type = chunk.chunk_type();

                    if *other_type == chunk_type {
                        ret_chunk = Some(chunk.clone());
                        break;
                    }
                }
    ...
    fn as_bytes(&self) -> Vec<u8> {
        ...
        bytes.extend(Self::STANDARD_HEADER);
    ...
    fn try_from(bytes: &[u8]) -> Result<Self, Self::Error> {
        ...
        loop {
            ...
            let length = bytes[pos..]
                .first_chunk::<4>()
                .ok_or::<Self::Error>("Expected 4 bytes of length field for chunk".into())?;

            let chunk_size = 12 + u32::from_be_bytes(*length) as usize;
